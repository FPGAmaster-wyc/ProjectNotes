...
任务模式有三种，数据存储任务(编号03），批处理任务（编号04）,数据检索任务（05）。先执行数据存储任务，接收到的数据会放在/var/opt/SatService/payload/in。然后执行一次批处理任务，会把输出数据放在/var/opt/SatService/payload/out，然后执行数据检索任务，就会把out下的数据发送出来
